...
export default function TurmasPage() {
  ...
  async function handleSave() {
    ...
    const supabase = createClient();
    if (dialog.editId) {
      await supabase.from("classes").update({ name: form.name, grade: form.grade, shift: form.shift, unit_id: form.unit_id }).eq("id", dialog.editId);
    } else {
      await supabase.from("classes").insert({ name: form.name, grade: form.grade, shift: form.shift, unit_id: form.unit_id, year: new Date().getFullYear() });
    }
  ...
  return (
    <div>
      <PageHeader
        title="Turmas"
        subtitle={loading ? "Carregando..." : `${classes.length} turma${classes.length !== 1 ? "s" : ""} ativa${classes.length !== 1 ? "s" : ""}`}
      >
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.unit_id} onValueChange={(v) => setForm((p) => ({ ...p, unit_id: v }))}>
                <SelectTrigger>
                  <SelectValue>{form.unit_id ? units.find(u => u.id === form.unit_id) ? `${units.find(u => u.id === form.unit_id).name} - ${units.find(u => u.id === form.unit_id).address}` : form.unit_id : "Selecione"}</SelectValue>
                </SelectTrigger>
